...
impl ListToolsResult {
    pub fn new() -> Self {
        Self { tools: Vec::new(), next_cursor: None }
    }
...
impl CallToolResult {
    pub fn new() -> Self {
        Self { content: Vec::new(), is_error: None, structured_content: None, meta: None }
    }
    ...
    pub fn with_text_content(mut self, text: impl Into<String>) -> Self {
        self.content.push(Content::Text(TextContent { text: text.into(), annotations: None, _meta: None }));
        self
    }
...
impl Tool {
    pub fn new(name: impl Into<String>, input_schema: ToolInputSchema) -> Self {
        Self { name: name.into(), input_schema, description: None, title: None, output_schema: None, annotations: None, _meta: None }
    }
    ...
    pub fn with_read_only_hint(mut self, read_only: bool) -> Self {
        self.annotations.get_or_insert_with(Default::default).read_only_hint = Some(read_only);
        self
    }

    pub fn with_destructive_hint(mut self, destructive: bool) -> Self {
        self.annotations.get_or_insert_with(Default::default).destructive_hint = Some(destructive);
        self
    }

    pub fn with_idempotent_hint(mut self, idempotent: bool) -> Self {
        self.annotations.get_or_insert_with(Default::default).idempotent_hint = Some(idempotent);
        self
    }

    pub fn with_open_world_hint(mut self, open_world: bool) -> Self {
        self.annotations.get_or_insert_with(Default::default).open_world_hint = Some(open_world);
        self
    }
...
impl ToolInputSchema {
    ...
    pub fn with_required_properties(mut self, names: impl IntoIterator<Item = impl Into<String>>) -> Self {
    ...
    pub fn from_json_schema<T: schemars::JsonSchema>() -> Self {
        ...
        let properties = schema_obj
            ...
            .map(|props| props.iter().map(|(k, v)| (k.clone(), v.clone())).collect::<HashMap<_, _>>());
        let required = schema_obj
            .and_then(|obj| obj.get("required"))
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect::<Vec<_>>());
        Self { schema_type, properties, required }
    }
}
